...
scripts/patch_diffusers_sana.py  local Diffusers SANA training script patch
```
...
The local Diffusers checkout is required. The SANA DreamBooth script is patched to read local folders containing images and `metadata.jsonl` directly.
...
- `make setup` runs `scripts/patch_diffusers_sana.py`, which patches the local Diffusers SANA script to load plain local folders with `metadata.jsonl`.
- If the local Diffusers checkout changes, rerun `make patch-diffusers`.
